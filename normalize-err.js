...
readFile("error-rate-SRS.txt","Ethernet","Random");
...
function readFile(infile, type, id){
	...
	//console.log(n+" : "+line);
		...
		if(!isNaN(data[1])){
			if(Number(data[1])>0)nfd_err.push(Number(data[1]));
		}
		if(!isNaN(data[2])){
			if(Number(data[2])>0)ndp_err.push(Number(data[2]));
		}
		...
		if(last){
			...
			for (var i = 0 ; i < nfd_err.length; i++) {
				fs.appendFile(outfile,type+"\t"+id+"\t"+nfd_err[i]+"\t"+ndp_err[i]+"\n",function(err){
