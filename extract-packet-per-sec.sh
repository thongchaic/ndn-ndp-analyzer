...
rm -f $OUTFILE
 ...
 do
    ...
    do
      ...
      NFDL="";
      for lt in $(cat res/$NFD/$i/ndnperf-Consumer$c | grep RES | awk '{print $11}')
      do
        NFDL="$NFDL $lt";
      done 

      NDPL="";
      for lt in $(cat res/$NDP/$i/ndnperf-Consumer$c | grep RES | awk '{print $11}')
      do
        NDPL="$NDPL $lt";
      done 
  ...
  echo ./extract-packet-per-sec.sh 9000
fi